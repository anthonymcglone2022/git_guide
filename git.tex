...
Git was written by Linus Torvalds who needed a tool to manage contributions from thousands of Linux Developers. At its core, Git is a repository which is used to store files. It's also used to version code for software releases.
\newline
\newline
...
First create a folder called \texttt{store}. If you're on MacOS or Linux, create it under the \texttt{/home/} folder, so your \texttt{path} to the folder will be \texttt{/home/store}. If you're on Windows create it under the \texttt{C} folder (when you're navigating to it the \texttt{path} will be \texttt{/c/store}). Now, open a terminal and navigate into that folder using the \texttt{cd} command (i.e. \texttt{cd /c/store}). 
\newline
\newline
...
\newline
\newline
Now commit these changes by running \texttt{git commit -m "This is my first commit!"} You can run \texttt{git log} after to see the commit ID. The ID is the code after the word \texttt{commit}:
\newline
\newline
\texttt{
...
\newline
\newline
To see if your local repository was updated, run \texttt{git remote -v} in your terminal. You should see \texttt{git@github.com:username/store.git}. Now, push your code to the remote repository (i.e. your remote master branch). Run the following:
\newline
\newline
...
\texttt{mysecondfile.txt} is also gone. This is what a hard reset does. A soft reset \texttt{git reset --soft HEAD{\raise.17ex\hbox{$\scriptstyle\mathtt{\sim}$}}1} would undo the latest commit but keep the files associated with that commit (it will take you back in time to the point just before you made your last commit). We'll explore poweruser commands like these in more detail later.
\newline
\newline
...
\newline
\newline
Run \texttt{git log} to check the commit was successful. Now let's push our changes to remote. The following gives an overview for the commands below it:
\newline
...
\newline
\newline
Always switch to the branch you want to merge your changes into. We wanted to merge our work (i.e. the \texttt{first} branch) into master, so we ran our merge command while being on master. The \texttt{--squash} argument squashes multiple commits into one commit. That is why we had to run a second commit on the local master branch. It's good practice to squash your commits into one, because it makes it easier to read the commit version history when you run \texttt{git log}. Finally, you always give the name of the branch that has to be merged at the end of the command.
...
\newline
\newline
Open up the \texttt{mysecondfile.txt} and add the text \texttt{Have less meetings} on \textbf{line 1}. Make sure you only add the text to the first line. Save the file. When you are happy with the changes, run the add, commit, push flow as usual:
\newline
...
\newline
\newline 
Open the file \texttt{mysecondfile.txt} and add the text \texttt{Do more off site team building} on \textbf{line 1}. Save the file, then run the following commands to do the merge:
\newline
...
\newline
\newline
We need to fix the conflict that has arisen. Here's what our text file \texttt{mysecondfile.txt} looks like now:
\newline
\newline
\texttt{\small
<<<<<<< HEAD
...
\newline
\newline
Okay, we are ready to do the rebase. Navigate back to your own repo. Switch to the master branch and pull down the latest changes. Check the version history and compare them with the version history of your colleagues \texttt{store}. When you're ready, switch to your \texttt{testrebase} branch. Run the following command to rebase.
\newline
